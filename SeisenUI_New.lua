...
function Library:CreateTripleTextbox(parent, options1, options2, options3)
    local boxName1     = options1.Name or "Textbox 1"
    local placeholder1 = options1.Placeholder or ""
    local default1     = options1.Default or ""
    local callback1    = options1.Callback or function() end
    ...
    local boxName3     = options3.Name or "Textbox 3"
    local placeholder3 = options3.Placeholder or ""
    local default3     = options3.Default or ""
    local callback3    = options3.Callback or function() end
    local flag3        = options3.Flag
    local clearOnFocus3 = options3.ClearTextOnFocus ~= false
    
    local sharedLabel  = options1.Label or "Triple Textbox"
    ...
    -- 1. Left Field
    local leftField = Create("Frame", {
        Size = UDim2.new(0.333, -4, 0, 26), Position = UDim2.new(0, 0, 0, 16),
        BackgroundColor3 = self.Theme.InputBg, Parent = container
    }, { Create("UICorner", { CornerRadius = UDim.new(0, 8) }) })
    ...
    -- 2. Middle Field
    local middleField = Create("Frame", {
        Size = UDim2.new(0.333, -4, 0, 26), Position = UDim2.new(0.333, 2, 0, 16),
        BackgroundColor3 = self.Theme.InputBg, Parent = container
    }, { Create("UICorner", { CornerRadius = UDim.new(0, 8) }) })
    ...
    textbox2.Focused:Connect(function() Tween(middleStroke, { Color = self.Theme.Accent }) end)
    textbox2.FocusLost:Connect(function()
        Tween(middleStroke, { Color = self.Theme.Border })
        callback2(textbox2.Text)
    end)
    ...
    -- 3. Right Field
    local rightField = Create("Frame", {
        Size = UDim2.new(0.333, -4, 0, 26), Position = UDim2.new(0.666, 4, 0, 16),
        BackgroundColor3 = self.Theme.InputBg, Parent = container
    }, { Create("UICorner", { CornerRadius = UDim.new(0, 8) }) })
    ...
    local textbox3 = Create("TextBox", {
        Size = UDim2.new(1, -10, 1, 0), Position = UDim2.new(0, 8, 0, 0),
        BackgroundTransparency = 1, Text = default3,
        PlaceholderText = placeholder3, PlaceholderColor3 = self.Theme.TextMuted,
        TextColor3 = self.Theme.Text, Font = Enum.Font.Gotham, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left, ClearTextOnFocus = clearOnFocus3,
        TextTruncate = Enum.TextTruncate.AtEnd, ClipsDescendants = true,
        Parent = rightField
    })
    self:RegisterElement(textbox3, "Text", "TextColor3")

    textbox3.Focused:Connect(function() Tween(rightStroke, { Color = self.Theme.Accent }) end)
    textbox3.FocusLost:Connect(function()
        Tween(rightStroke, { Color = self.Theme.Border })
        callback3(textbox3.Text)
    end)

    local boxObj3 = {
        Value = textbox3.Text, Type = "Input",
        SetValue = function(s, v) textbox3.Text = v; s.Value = v; callback3(v) end
    }
    textbox3:GetPropertyChangedSignal("Text"):Connect(function() boxObj3.Value = textbox3.Text end)
    self:ApplyCommonProperties(container, options3, boxObj3)
    if flag3 then self.Options[flag3] = boxObj3 end

    return boxObj1, boxObj2, boxObj3
end
...
function Library:Unload()
    ...
    -- Explicit fly cleanup — kill the BodyVelocity/BodyGyro and restore PlatformStand
    ...
    self._flyVel = nil
    if self._flyGyro and self._flyGyro.Parent then
        pcall(function() self._flyGyro:Destroy() end)
    end
    self._flyGyro = nil
...
function Library:EnsureStatsWidget()
    if not self._statsWidget then
        self._statsWidget = self:CreateWidget({
            Title = "Performance",
            ShowFPS = true,
            ShowPing = true,
            ShowMemory = true,
...
function Library:CreateWindow(options)
    ...
    function Window:ShowChangelog(entries)
        ...
        Window._pendingChangelog = entries
        if not Library.IntroOngoing then
            if clEnabled() then
                Window:_renderChangelog(entries, nil)
            end
        end
        -- If IntroOngoing, the Phase-4 intercept in the intro spawn handles it.
    end
    ...
    function Window:_renderChangelog(entries, onClose, autoClose)
        ...
        local panel = Create("Frame", {
            ...
            AnchorPoint = Vector2.new(1, 0.5),
            Position = UDim2.new(1, PANEL_W + 10, 0.5, 0),
            BackgroundColor3 = Library.Theme.Background or Color3.fromRGB(18, 18, 24),
            BorderSizePixel = 0,
            ZIndex = 251,
            Parent = Library.ScreenGui,
        }, {
            Create("UICorner", { CornerRadius = UDim.new(0, 12) }),
            Create("UIStroke", { Color = Library.Theme.Border or Color3.fromRGB(50, 50, 70), Thickness = 1.2 }),
            Create("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 0) }),
        })
        ...
        local scroll = Create("ScrollingFrame", {
            ...
            AutomaticCanvasSize = Enum.AutomaticSize.Y,
            BorderSizePixel = 0,
            LayoutOrder = 3, ZIndex = 252, Parent = panel,
        }, {
            Create("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 0) }),
        })
        ...
        for i, entry in ipairs(entries) do
            ...
            local iconImg = Create("ImageLabel", {
                ...
                Position = UDim2.new(0.5, 0, 0.5, 0),
                BackgroundTransparency = 1,
                ImageColor3 = isLatest and ACCENT or (Library.Theme.TextDim or Color3.fromRGB(130, 130, 160)),
                ZIndex = 255, Parent = iconCircle,
            })
        ...
        Create("Frame", {
            ...
            BackgroundColor3 = Library.Theme.Border or Color3.fromRGB(50, 50, 70),
            BorderSizePixel = 0,
            LayoutOrder = 4, ZIndex = 252, Parent = panel,
        })

        -- Footer: "Show on startup" toggle + "Got it" button
        local footer = Create("Frame", {
            Size = UDim2.new(1, 0, 0, 46),
            BackgroundTransparency = 1,
            LayoutOrder = 5, ZIndex = 252, Parent = panel,
        }, {
            Create("UIPadding", {
        ...
        toggleBtn.MouseButton1Click:Connect(function()
            ...
            clSetEnabled(showEnabled)
            Tween(togglePill, { BackgroundColor3 = showEnabled and ACCENT or (Library.Theme.InputBg or Color3.fromRGB(40, 40, 55)) }, 0.15)
        ...
        Tween(panel, { Position = UDim2.new(1, -10, 0.5, 0) }, 0.3)
...
function Library:CreateSection(parent, name, iconName)
    ...
    function S:AddTripleTextbox(opts1, opts2, opts3)
        local f = Create("Frame", {
            Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1,
            AutomaticSize = Enum.AutomaticSize.Y, LayoutOrder = nextOrder(), Parent = container
        })
        return Library:CreateTripleTextbox(f, opts1, opts2, opts3)
    end
...
function Library:_BuildConfigTab(window)
    ...
    configLeft:AddToggle({
        ...
        Callback = function(v)
            ...
            -- Keeps the HRP upright/facing the camera so it doesn't tumble
            -- from physics torque once PlatformStand disables Humanoid rotation control.
            ...
            flyGyro.Parent = hrp
            self._flyGyro = flyGyro

            -- Disables collision on the whole character so bumping into geometry
            -- can't shove/flip the HRP hard enough to overpower the gyro.
            flyCollisions = {}
    ...
    configPerf:AddToggle({
        ...
        Callback = function(v)
            Library._perfHudEnabled = v
            if v then
                Library:EnsureStatsWidget()
                if Library._widgetGui then
                    Library._widgetGui.Enabled = true
                end
            else
                if Library._widgetGui and Library.ScreenGui and Library.ScreenGui.Enabled then
                    Library._widgetGui.Enabled = false
                end
            end
        end
    })
...
function Library:_BuildSuggestionsTab(window)
    ...
    suggestSection:AddDropdown({
        Name     = "Category",
        Options  = { "Vouch", "Suggestion", "Bug Report" },
        Default  = "Vouch",
        Flag     = "BuiltIn_SuggestCategory",
        Tooltip  = "Pick the category that best fits your message.",
        Callback = function(v) suggestCategory = v or "Vouch" end,
    })
    ...
    suggestSection:AddButton({
        ...
        Callback = function()
            ...
            local ok = pcall(function()
                local HS = game:GetService("HttpService")
                -- Anonymous by design: no username, userId, or avatar is ever read or sent,
                -- so submissions can't be traced back to a specific player.
                ...
                elseif suggestCategory == "Bug Report" then
                    body = HS:JSONEncode({
                        thread_name = "[Bug Report] " .. isoTime,
                        username    = "Seisen Hub",
                        embeds = {{
                            title       = "Bug Report",
                            ...
                            footer      = { text = "Submitted anonymously" },
                        }},
                    })
                else
                    body = HS:JSONEncode({
                        thread_name = "[Suggestion] " .. isoTime,
                        username    = "Seisen Hub",
                        embeds = {{
                            title       = "Suggestion",
                            description = suggestText,
